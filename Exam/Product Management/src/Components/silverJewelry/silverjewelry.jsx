...
const SilverJewelryCardData = () => {
  ...
  // Filter only Silver Jewelry category products
  const silverJewelryProducts = useMemo(() => {
    if (!Array.isArray(products)) return [];
    return products.filter(
      (product) =>
        product.category &&
        product.category.trim().toLowerCase() === "silverjewelry"
    );
  }, [products]);

  useEffect(() => {
    setFilteredProducts(silverJewelryProducts);
  }, [silverJewelryProducts]);
  ...
  const applyFilters = (brands, sizes) => {
    let filtered = silverJewelryProducts;
    ...
    if (sizes.length > 0) {
      filtered = filtered.filter((item) => {
        if (Array.isArray(item.size)) {
          return item.size.some((s) => sizes.includes(s.trim().toUpperCase()));
  ...
  const allSizes = useMemo(
    () =>
      [
        ...new Set(
          silverJewelryProducts.flatMap((p) =>
          ...
          <div className="d-flex flex-wrap gap-4 justify-content-start">
            {filteredProducts.length > 0 ? (
              ...
              <h5 className="text-center w-100 mt-5">
                No Silver Jewelry products found
              </h5>
            )}
          </div>
        </Col>
...
export default SilverJewelryCardData;
