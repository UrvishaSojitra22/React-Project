...
export const getAllProductsAsync = () => {
  return async (dispatch) => {
    ...
    try {
      let res = await getDocs((collection(db, "Product Menegement")));
      let result = [];
      res.forEach((doc) => {
        const data = doc.data();
        result.push(data);
...
export const addNewProductAsync = (data) => {
  return async (dispatch) => {
    ...
    try {
      let res = await setDoc(doc(db, "Product Menegement", `${data.id}`), data);
...
export const deleteProductAsync = (id) => {
  return async (dispatch) => {
    ...
    try {
      await deleteDoc(doc(db, "Product Menegement", `${id}`));
...
export const getProductAsync = (id) => {
  return async (dispatch) => {
    ...
    try {
      let res = await getDoc(doc(db, "Product Menegement", `${id}`))
...
export const updateProductAsync = (data) => {
  return async (dispatch) => {
    ...
    //   );
    try {
      await updateDoc(doc(db, "Product Menegement", `${data.id}`), data)
