...
const KidsCardData = () => {
    ...
    // ----------------------------------------------------
    // 1. BASE FILTERING: Filter products to show ONLY 'Kids' category products
    const kidsCategoryProducts = useMemo(() => {
        if (!Array.isArray(products)) return [];

        return products.filter(product =>
            product.category && product.category.trim().toLowerCase() === 'kids'
        );
    }, [products]);

    // Set initial filtered products when kidsCategoryProducts changes
    useEffect(() => {
    ...
    // ----------------------------------------------------
    // 2. FILTERING LOGIC UPDATE: Use kidsCategoryProducts as the base for filters
    const applyFilters = (brands, sizes) => {
        let filtered = kidsCategoryProducts; // Base filtering set to Kids' products
    ...
    ].filter(Boolean), [kidsCategoryProducts]);
                ...
                <Col md={3} sm={12} className="border-end p-3">
                    ...
                    <Button
                        ...
                        onClick={() => {
                            setSelectedBrands([]);
                            setSelectedSizes([]);
                            // Clear filters sets the view back to ALL Kids' products
                            setFilteredProducts(kidsCategoryProducts);
                        }}
                    >
                        Clear Filters
                ...
                <Col md={9} sm={12}>
                    <div className="d-flex flex-wrap gap-4 justify-content-start">
                        {filteredProducts.length > 0 ? (
                            ...
                            <h5 className="text-center w-100 mt-5">No Kids' products found</h5>
                        )}
                    </div>
                </Col>
            </Row>
