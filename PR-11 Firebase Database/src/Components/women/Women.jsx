...
const WomenCardData = () => {
  ...
  const { products } = useSelector((state) => state); // All products from Redux
  ...
  // ----------------------------------------------------
  // 1. BASE FILTERING: Filter products to show ONLY 'Women' category products
  const womenCategoryProducts = useMemo(() => {
    if (!Array.isArray(products)) return [];

    return products.filter(product =>
      product.category && product.category.trim().toLowerCase() === 'women'
    );
  }, [products]);

  // Set initial filtered products when womenCategoryProducts changes
  useEffect(() => {
    setFilteredProducts(womenCategoryProducts);
  ...
  // ----------------------------------------------------
  // 2. FILTERING LOGIC UPDATE: Use womenCategoryProducts as the base for filters
  const applyFilters = (brands, sizes) => {
    let filtered = womenCategoryProducts; // Base filtering set to Women's products
        ...
        <Col md={3} sm={12} className="border-end p-3">
          ...
          <Button
            ...
            onClick={() => {
              setSelectedBrands([]);
              setSelectedSizes([]);
              // Clear filters sets the view back to ALL Women's products
              setFilteredProducts(womenCategoryProducts);
            }}
          >
            Clear Filters
        ...
        <Col md={9} sm={12}>
          <div className="d-flex flex-wrap gap-4 justify-content-start">
            {filteredProducts.length > 0 ? (
              ...
              <h5 className="text-center w-100 mt-5">No Women's products found</h5>
            )}
          </div>
        </Col>
      </Row>
