...
const WomenCardData = () => {
  ...
  const { products } = useSelector((state) => state.productReducer); // All products from Redux
  ...
  const applyFilters = (brands, sizes) => {
    let filtered = womenCategoryProducts;
            ...
            {filteredProducts.length > 0 ? (
              filteredProducts.map((item) => (
                <Card
                  ...
                  onClick={() => handleProductClick(item.id)} // ✅ click = single product page
                >
                  <Card.Img
                    variant="top"
                    src={item.image}
